...
def fetch_website(urllib_version, url):
    # Import only explicitly allowed urllib versions safely
    version = str(urllib_version).strip()
    if version not in {"2", "3"}:
        raise ValueError("Unsupported urllib version")

    # urllib.PoolManager is provided by urllib3; keep behavior while removing exec
    urllib_module = importlib.import_module("urllib3")
